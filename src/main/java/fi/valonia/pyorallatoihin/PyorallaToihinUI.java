...
public class PyorallaToihinUI extends UI {
    ...
    protected void init(VaadinRequest request) {
        ...
        if ("/admin".equals(pathInfo)) {
            setLanguage(null);
            Page.getCurrent().setTitle(messages.getString(Messages.header_topic));
            showManageScreen();
        } else {
            ...
            Page.getCurrent().addUriFragmentChangedListener(new UriFragmentChangedListener() {
                ...
                public void uriFragmentChanged(UriFragmentChangedEvent event) {
                    parseFragment(event.getUriFragment());
                    openFragmentView();
                }
            });
            openFragmentView();
    ...
    public static PyorallaToihinUI get() {
    	UI ui = UI.getCurrent();
    	if(ui instanceof PyorallaToihinUI){
    		return (PyorallaToihinUI) ui;
    	}
    	return null;
    }
    ...
    private void setFragment(boolean fireEvent) {
        ...
        Page.getCurrent().setUriFragment(fragment.toString(), fireEvent);
    }
