...
public class EmployeeRow extends CssLayout {
    ...
    private void paintRow() {
        ...
        Label sport = new Label(PyorallaToihinUI.get()
    ...
    private boolean toggleDay(int day) {
        ...
        if (daysMarked > 5 && employee.getDays()[day]) {
        	Notification.show(ui.getMessages().getString(
                    Messages.max_five_days_to_stats), Type.TRAY_NOTIFICATION);
        }
    ...
    private int getTodayNumber() {
        ...
        ISystemService systemService = PyorallaToihinUI.get().getSystemService();
